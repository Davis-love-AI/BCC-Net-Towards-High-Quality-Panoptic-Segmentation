...
class HybridTaskCascade(CascadeRCNN):
    ...
    def __init__(self,
        ...
        if semantic_head is not None:
            ...
            self.mask_pred_roi_extractor = builder.build_roi_extractor(
                mask_pred_roi_extractor)
            self.bbox_pred_roi_extractor = builder.build_roi_extractor(
                bbox_pred_roi_extractor)
    ...
    def _bbox_forward_train(self,
        ...
        if self.with_semantic and 'bbox' in self.semantic_fusion:
            ...
            bbox_pred_feat = self.bbox_pred_roi_extractor([semantic_pred], rois)
            bbox_feats = torch.cat([bbox_feats, bbox_pred_feat], dim=1)
    ...
    def _mask_forward_train(self,
        ...
        if self.with_semantic and 'mask' in self.semantic_fusion:
            ...
            # SMP: project semantic mask for mask prediction
            mask_pred_feat = self.mask_pred_roi_extractor([semantic_pred], pos_rois)
    ...
    def _bbox_forward_test(self, stage, x, rois, semantic_feat=None, semantic_pred=None):
        bbox_roi_extractor = self.bbox_roi_extractor[stage]
        bbox_head = self.bbox_head[stage]
    ...
    def _mask_forward_test(self, stage, x, bboxes, semantic_feat=None,semantic_pred=None):
        mask_roi_extractor = self.mask_roi_extractor[stage]
        mask_head = self.mask_head[stage]
        ...
        if self.with_semantic and 'mask' in self.semantic_fusion:
            ...
            if mask_semantic_feat.shape[-2:] != mask_feats.shape[-2:]:
                mask_semantic_feat = F.adaptive_avg_pool2d(
                    mask_semantic_feat, mask_feats.shape[-2:])
            mask_feats += mask_semantic_feat
            mask_pred_feat = self.mask_pred_roi_extractor([semantic_pred], mask_rois)
            mask_feats = torch.cat([mask_feats, mask_pred_feat], dim=1)
        if self.mask_info_flow:
            last_feat = None
            last_pred = None
