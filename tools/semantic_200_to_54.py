...
def extract_semantic_single_core(proc_id,
    ...
    for working_idx, img_name in enumerate(img_name_set):
        ...
        semantic_54_img = np.zeros(semantic_200_img.shape, dtype=np.uint8)

        unique_cat_list = list(np.unique(semantic_200_img))
        for cat in unique_cat_list:
            mask = semantic_200_img == cat
            semantic_54_img[mask] = cat_200_to_54[cat]

        Image.fromarray(semantic_54_img).save(os.path.join(folder_54, img_name))

    print('Core: {}, all {} images processed'.format(proc_id, len(img_name_set)))
    return []


def extract_semantic(folder_200, folder_54):
    ...
    for idx, data in enumerate(cat_json[:80]):
        cat_200_to_54[data['id']] = 0  # other class 0
    for idx, data in enumerate(cat_json[80:]):
        cat_200_to_54[data['id']] = idx + 1  # stuff class: mapped to 1 - 53

    print("category mapping from 200 to 54:")
    print(cat_200_to_54)

    if not os.path.exists(folder_54):
        os.mkdir(folder_54)
    ...
    for proc_id, img_name_set in enumerate(img_name_split):
        p = workers.apply_async(extract_semantic_single_core,
                                (proc_id, list(img_name_set), folder_200, folder_54, cat_200_to_54))
        processes.append(p)
    temp = []
    for p in processes:
        temp.extend(p.get())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--semantic_seg_folder_200', type=str, default=None,
                        help="input folder")
    parser.add_argument('--semantic_seg_folder_54', type=str, default=None,
                        help="output folder")
    args = parser.parse_args()
    extract_semantic(args.semantic_seg_folder_200, args.semantic_seg_folder_54)
